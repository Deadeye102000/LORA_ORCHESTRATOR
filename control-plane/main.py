...
# ─────────────────────────────────────────────────────────────
#  HEARTBEAT FAILOVER BACKGROUND TASK
#  Runs every 10 seconds. Marks workers offline if last_seen
#  is older than 30 seconds, and reverts their active jobs to
...
async def heartbeat_monitor():
    """Background task: detects stale workers and reverts their jobs."""
    print("[HeartbeatMonitor] Background sweep task started.")
    while True:
        ...
        with SessionLocal() as db:
            stale_workers = (
                db.query(Worker)
                .filter(Worker.last_seen < cutoff, Worker.status != "offline")
                .all()
            )
            for worker in stale_workers:
                print(
                    f"[HeartbeatMonitor] Worker '{worker.id}' is stale "
                    f"(last_seen={worker.last_seen}). Marking offline."
                )
                worker.status = "offline"

                # Revert any TRAINING jobs assigned to this worker back to PENDING
                stuck_jobs = (
                    db.query(Job)
                    .filter(Job.status == "TRAINING", Job.worker_id == worker.id)
                    .all()
                )
                for job in stuck_jobs:
                    print(
                        f"[HeartbeatMonitor] Reverting Job {job.id} "
                        f"from TRAINING → PENDING (worker '{worker.id}' "
                        "went offline)."
                    )
                    job.status = "PENDING"
                    job.worker_id = None
                ...
                # Broadcast offline event so the UI updates immediately
...
async def lifespan(app: FastAPI):
    ...
    yield
    # Clean shutdown: cancel the monitor task when the app exits
    task.cancel()
    try:
        await task
    except asyncio.CancelledError:
        print("[HeartbeatMonitor] Background task cancelled on shutdown.")
...
app = FastAPI(title="Distributed LoRA Training Orchestrator", lifespan=lifespan)
...
class WorkerHeartbeatRequest(BaseModel):
    ...
    status: str = Field(
        "idle",
        description="Current status of the worker (idle, busy, offline)",
    )
...
class JobCreateRequest(BaseModel):
    base_model: str = Field(
        ...,
        description="Base model name/path (e.g. TinyLlama/...)",
    )
    dataset_url: str = Field(..., description="URL or path to training dataset")
    hyperparameters: Dict[str, Any] = Field(
        default_factory=dict,
        description="JSON dictionary of LoRA hyperparameters",
    )
    min_vram_gb: float = Field(
        default=0.0,
        description="Minimum worker VRAM (GB) required to run this job",
    )
...
class WorkerPollRequest(BaseModel):
    ...
    gpu_vram_gb: float = Field(
        default=0.0,
        description="Worker's available GPU VRAM in GB for matching",
    )
...
class JobUpdateRequest(BaseModel):
    ...
    weights_path: Optional[str] = Field(
        None, description="Path to trained adapter weights"
    )
...
async def worker_heartbeat(
    ...
    await manager.broadcast_to_room(
        "system", {"event": "worker_update", "worker": worker_data}
    )
...
def poll_job(payload: WorkerPollRequest, db: Session = Depends(get_db)):
    """
    VRAM-aware scheduling: only assign jobs whose min_vram_gb requirement
    is satisfied by the polling worker's gpu_vram_gb capacity.
    Jobs are dispatched in FIFO order (oldest PENDING first).
    """
    job = (
        db.query(Job)
        .filter(
            Job.status == "PENDING",
            Job.min_vram_gb <= payload.gpu_vram_gb,
        )
...
async def add_job_metric(
    job_id: int, payload: MetricCreateRequest, db: Session = Depends(get_db)
):
    ...
    if (
        payload.step is not None
        and payload.loss is not None
        and payload.epoch is not None
    ):
        db_metric = Metric(
            job_id=job_id,
            step=payload.step,
...
def update_job(job_id: int, payload: JobUpdateRequest, db: Session = Depends(get_db)):
    ...
    if payload.status in ["COMPLETED", "FAILED"]:
        job.completed_at = datetime.datetime.now(datetime.timezone.utc).replace(
            tzinfo=None
        )
...
async def websocket_system(websocket: WebSocket):
    ...
    try:
        await websocket.send_json({"event": "init_workers", "workers": workers_list})
        while True:
